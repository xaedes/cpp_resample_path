...
set TESTS_PROJECT=cpp_resample_path_tests
...
rem ---------------------------------------------------------------------------
:tools
rem echo Setup Compiler Tools...
rem call %SETUP_COMPILERS_CMD% %SETUP_COMPILERS_TRIPLET%

rem echo Preparing tools...
rem if not exist "%DIR%\tools\" (
rem     mkdir "%DIR%\tools\"
rem )
rem if not exist "%DIR%\tools\vcpkg\vcpkg" (
rem     git clone --depth 1 --single-branch --branch imgui-cpp11 https://github.com/xaedes/vcpkg.git "%DIR%\\tools\\vcpkg/"
rem     cd "%DIR%\\tools\\vcpkg\\"
rem     git pull origin imgui-cpp11
rem     cd "%DIR%\\"
rem     rem git clone https://github.com/microsoft/vcpkg.git "%DIR%\\tools\\vcpkg\\"
rem     "%DIR%\tools\vcpkg\bootstrap-vcpkg.bat" -disableMetrics
rem )
goto exit
...
cmake --version
rem cmake -G "%CMAKE_GENERATOR%" -DCMAKE_BUILD_TYPE=%BUILD_TYPE% -DCMAKE_TOOLCHAIN_FILE=%DIR%\tools\vcpkg\scripts\buildsystems\vcpkg.cmake %DIR%
